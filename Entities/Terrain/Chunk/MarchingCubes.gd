...
func interpolate_vertex(voxel_a : Dictionary, voxel_b : Dictionary):
	if abs(density_threshold - voxel_a.density) < ChunkProps.FLOAT_THRESHOLD:
		return Vector3(voxel_a.x, voxel_a.y, voxel_a.z)
	if abs(density_threshold - voxel_b.density) < ChunkProps.FLOAT_THRESHOLD:
		return Vector3(voxel_b.x, voxel_b.y, voxel_b.z)
	if abs(voxel_a.density - voxel_b.density) < ChunkProps.FLOAT_THRESHOLD:
		return Vector3(voxel_a.x, voxel_a.y, voxel_a.z)
